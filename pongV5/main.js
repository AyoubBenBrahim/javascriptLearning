...
function shiftPaddlesVertical(ball) {
  ...
  let shiftRatio = canvasHeight * 0.09;
  if (
...
class Paddle {
  ...
  // updateAttributes = (canvasWidth, canvasHeight) => {};

  initialize(canvasWidth, canvasHeight) {
 
    if (canvasWidth >= canvasHeight) {

      this.paddleHeight = 0.22; // 20% of canvas height
      this.paddleWidth = 0.012; // 2% of canvas width
      this.margin = 0.024; // 5% of canvas width for the margin

      this.paddleX = this.side === "left" ? this.margin * canvasWidth 
                    : canvasWidth - (this.margin * canvasWidth) - this.paddleWidth * canvasWidth;
        
      this.paddleY = (canvasHeight - this.paddleHeight * canvasHeight) / 2;
    } else {
      this.paddleHeight = 0.12; // 20% of canvas height
      this.paddleWidth = 0.012; // 2% of canvas width
      this.margin = 0.024; // 5% of canvas width for the margin

      this.paddleX = this.side === "left" ? this.margin * canvasHeight
      : canvasHeight - (this.margin * canvasHeight) - this.paddleWidth * canvasHeight;
        
      this.paddleY = canvasWidth / 2 - (this.paddleHeight * canvasHeight) / 2;
    }
...
if (landscapeModeView) {
  paddleHeight = 0.22; //0.2;////0.0003 * canvasHeight; // 20% of canvas height
  paddleWidth = 0.012; //0.012; // 2% of canvas width
  margin = 0.024; // 5% of canvas width for the margin
  ...
  paddleHeight = 0.12; //0.2;////0.0003 * canvasHeight; // 20% of canvas height
  paddleWidth = 0.012; //0.012; // 2% of canvas width
  margin = 0.024; // 5% of canvas width for the margin
...
const paddleOptions = {
  ...
  radius: canvasWidth * 0.01,//0.012, // % of canvas width
  velocity: 50,
  color: "tomato",
};
...
function draw() {
  ...
  // paddleLeft.initialize(canvasWidth, canvasHeight);
  // paddleRight.initialize(canvasWidth, canvasHeight);
  ball.update();
}
